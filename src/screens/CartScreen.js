        ...
        <>
                        ...
                        {realWishlistProducts.map(item => (//for each item in cartItems add ListGroup.Item. and we add key=item.product(id) which is id
                            <ListGroup.Item key={item.id}>
                                <Row>
                                    ...
                                    <Col md={2}>
                                        <Form.Control as='select' value={item.countInStock}>
            ...
            <Col md={4}>
                <Card>
                    <ListGroup variant='flush'>
                        <ListGroup.Item>
                            <h2>Subtotal ({realWishlistProducts.reduce((acc, item) => acc + item.countInStock, 0)})
                            items</h2>
                            ${realWishlistProducts.reduce((acc, item) => acc + item.countInStock * item.price, 0).toFixed(2)}
                        </ListGroup.Item>
                    </ListGroup>
                </Card>
            </Col>
